...
// Function to overwrite environment variables
...
async function updateEnv(variable, value) {
  ...
  fs.readFile(envFilePath, "utf8", (err, data) => {
    ...
    // Check if the variable doesn't exist in the file, add it to the end
    if (!regex.test(data)) {
      newData += `\n${variable}=${value}`;
    }
...
function loadData(file) {
  try {
    ...
    if (datas?.length <= 0) {
      console.log(colors.red(`Could not find data in ${file}`));
      return [];
    }
    return datas;
  } catch (error) {
