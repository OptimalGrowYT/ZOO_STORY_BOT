...
console.log(colors.yellow('PASTE YOUR (Query ID) INTO Query_ID.TXT FILE AND PRESS START'));
console.log(colors.green('                     [𝍖𝍖𝍖 ZOO FAMILY AIRDROP 𝍖𝍖𝍖]                     '));
...
class ZooAPIClient {
  ...
  async checkProxyIP(proxy) {
    try {
      ...
      if (response.status === 200) {
        ...
        throw new Error(`❌ Cannot check IP of the proxy 🌐. Status code: ${response.status}`);
      }
    } catch (error) {
      throw new Error(`Error when checking the proxy's IP ⚠️: ${error.message}`);
    }
  }
  ...
  async AnswerDaily(hash, accountIndex, questKey, checkData) {
    ...
    try {
      ...
      if (response.status === 200 && response.data.success) {
        ...
        this.log(`Check task 📋"${questKey}" Failure 😞: ${response.data.error}`, "warning");
        return { success: false, error: response.data.error };
      }
    } catch (error) {
      this.log(`Error when checking the task ⚠️ "${questKey}": ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
  ...
  async claimQuest(hash, accountIndex, questKey, checkData = null) {
    ...
    try {
      ...
      if (response.status === 200 && response.data.success) {
        this.log(`Claim task 🎯 "${questKey}" Success, claim reward 🎉.`, "success");
        return { success: true, data: response.data };
      } else {
        return { success: false, error: response.data.error };
      }
    } catch (error) {
      this.log(`Error when claiming the task ⚠️ "${questKey}": ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
    ...
    try {
      ...
      for (const quest of quests) {
        ...
        if (quest.checkType === "checkCode") {
          this.log(`Start completing the daily question-answering task...`, "custom");
        ...
        if (claimResult.success === true) {
          this.log(`Complete the task ✅ ${quest.key} | "${quest.title}", Receive 🎁 ${quest.reward} Reward 🎉.`, "success");
        } else if (claimResult.error === "already rewarded") {
          this.log(`Task 📝 ${quest.key} "${quest.title}" Has been completed earlier ✔️`, "warning");
        } else {
          this.log(`Cannot complete or needs to be done manually ❌ ${quest.key} | "${quest.title}": ${claimResult.error}`, "warning");
        }
        await new Promise((resolve) => setTimeout(resolve, 1000));
      }
    } catch (error) {
      this.log(`Error when fetching the task list ⚠️: ${error.message}`, "error");
    }
  }
          ...
          if (dbAnimal) {
            ...
            if (nextLevelData && hero.coins >= nextLevelData.price) {
              ...
              try {
                ...
                if (response.status === 200 && response.data.success) {
                  this.log(`Upgrade 🔧 ${dbAnimal.title} Successfully leveled up ⬆️ ${nextLevel}`, "success");
                }
              } catch (error) {
  ...
  async main() {
    try {
      ...
      if (!hasIDAPI) return console.log(`Cannot find API ID, please try again later! ❌`.red);
      console.log(`${message}`.yellow);
      this.baseURL = hasIDAPI;
      const data = loadData("Query_ID.txt");
      ...
      while (true) {
            ...
            if (loginResult.success) {
              ...
              if (userDataResult.success) {
                ...
                if (i === 0 && !feed.isNeedFeed && feed.nextFeedTime) {
                  ...
                  this.log(`Time for the next feeding ⏰: ${localFeedTime.toFormat("yyyy-MM-dd HH:mm:ss")}`, "info");
                }
                ...
                if (dataAfterResult.success) {
                  ...
                  for (let day = 1; day <= 16; day++) {
                    if (dailyRewards[day] === "canTake") {
                      this.log(`Receiving daily reward 🎁🌟 ${day}...`, "info");
        ...
        if (firstAccountFeedTime) {
          ...
          this.log(`Use default wait time of 24 hours ⏳`, "info");
        }

        await this.countdown(nextCycleWaitTime); // Use countdown here
      }
    } catch (error) {
